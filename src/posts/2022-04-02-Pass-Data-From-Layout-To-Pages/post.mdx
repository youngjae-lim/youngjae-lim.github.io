...
In the previous article, we went over three ways to pass data from a parent page(layout page) to a child page and a grandchild component. The example was quite limited because we hard-coded data in the layout page and then pass that into the child page and the grand child component. More realistic case would be fetching the data from a backend api and pass that into other pages. In a new example in this article, we will fetch data from an endpoint and load it into a layout page so that we can access the same data from any pages that are wrapped around by the layout page.
