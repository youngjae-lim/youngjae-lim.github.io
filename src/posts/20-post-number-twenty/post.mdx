...
### Create a `./src/routes/__layout.svelte`:
...
Create `./src/components/PostCard.svelte` file:
...
- Replace `<p>` tag with `<PostCard {post} />` (line 59)

```js:./src/routes/blog/index.svelte highlights=50,59
<script context="module">
  const query = `
    query getPosts {
      posts {
...
Create a `./src/components/PodcastPlayer.svelte` file:
...
Render this component in our `./src/__layout.svelte` component.
